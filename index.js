...
function makePersonObject(arg1, arg2, arg3) {
  const object = {id:arg1, name:arg2, email:arg3};
  return object;
}
...
function getName(obj) {
  let str= 'Hello, my name is ' + obj.name;
  return str;
}
...
function getLastCarInfo(arr) {
  const makeModel = arr.find((item, index) => {
...
function getModelYears(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let year = arr.find((item, index) => {
      return index === i;
    })
    yearsArr[i] = year.car_year;
  }
  return yearsArr;
}
...
function getGermanCars(arr) {
  let newArr = [];
  for (let i = 0; i < arr.length; i++) {
    let make = arr.find((item, index) => {
      return index === i;
    })
    if (make.car_make == 'Audi') {
      newArr.push(make);
    } else if (make.car_make == 'Mercedes-Benz') {
      newArr.push(make);
    } else if (make.car_make == 'Volkswagen') {
      newArr.push(make);
    } else if (make.car_make == 'BMW') {
      newArr.push(make);
    }
  }
  return newArr;
